% định ngĩa hàm và khai báo các thông só đầu vào N1, N2, d, P1, P2 và Delta
function [codebook,record] = generate_near_field_codebook(N1,N2,d,P1,P2,Delta)

N=N1*N2; % tính số phần tử của bảng mã

% trích xuất các giá trị tọa độ theo các vector P1 và P2
...
% trích xuất giá trị tọa độ từ vector Delta
...
% tạo lưới tọa độ x, y, z cho RIS bằng các giá trị min tương ứng cho mỗi chiều x, y và z
...
% tính số điểm lưới tọa độ trên mỗi chiều x, y và z
Xnum1=length(Xgrid1); Ynum1=length(Ygrid1); Znum1=length(Zgrid1);
Xnum2=length(Xgrid2); Ynum2=length(Ygrid2); Znum2=length(Zgrid2);

% tạo mảng lưu giá trị các tọa độ điểm lưới và mã codebook tương ứng
record=zeros(Xnum1*Ynum1*Znum1*Xnum2*Ynum2*Znum2,6);
codebook = zeros(Xnum1*Ynum1*Znum1*Xnum2*Ynum2*Znum2,N);

i=1; % khởi tạo biến chạy i
% vòng lặp qua từng điểm lưới của mảng kênh thứ 1 ( BS - RIS )
for x1=Xgrid1
    for y1=Ygrid1
        for z1=Zgrid1
% vòng lặp qua từng điểm lưới của mảng kênh thứ 2 ( RIS - UE )
            for x2=Xgrid2
                for y2=Ygrid2
                    for z2=Zgrid2
% trong vòng lặp ta tạo vector a chứa pha tính hiệu bằng công thức kênh cận trường
                        a = zeros(1,N);
                        for n1=1:N1
                            for n2=1:N2
...
%                         a=a/sqrt(N);
                        codebook(i,:)=a; % lưu vector a vào hàng thứ I của codebook
                        record(i,:)=[x1,y1,z1,x2,y2,z2]; % gán giá trị vector a vào hàng thứ i của codebook và lưu tọa độ vào hàng thứ i của record
                        i=i+1;
                    end
                end
            end
...
[codebook,index]=unique(codebook,'row'); % bảng mã của codebook
record=record(index,:); % tập hợp các tọa độ và mã tương ứng trong codebook
end
